  ...
  return (
    <div className="p-6 bg-[#DFF6F5] min-h-screen font-sans">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
            onClick={() => setView(view === "grid" ? "list" : "grid")} className="w-10 h-10 flex items-center justify-center rounded-lg bg-[#7ED4D1] text-[#1D6F6B] hover:bg-[#279D99] hover:text-white transition-all"
            title={view === "grid" ? "Vue liste" : "Vue grille"}>
            {view === "grid" ? (<FiList size={20} />) : (<FiGrid size={20} />)}
          </button>
          {/* Create */}
          <button
            onClick={() => setShowModal(true)} className="h-10 px-4 rounded-lg bg-[#30B2AC] text-white hover:bg-[#1D6F6B]">
            + Create
          </button>
        </div>
      </div>
      ...
      {view === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[...Array(6)].map((_, index) => (
            <div
              key={index} className="bg-white rounded-xl border border-[#7ED4D1] shadow p-4">
              ...
              <div className="flex justify-end gap-2 mt-4">
                <button onClick={() => navigate(`/Cours/display/${index + 1}`)} className="px-3 py-1 rounded bg-[#30B2AC] text-white">
                  Display
                </button>
                <button
                  onClick={() =>navigate(`/Cours/update/${index + 1}`)} className="px-3 py-1 rounded bg-[#279D99] text-white">
                  Update
                </button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow border border-[#7ED4D1]">
          {[...Array(6)].map((_, index) => (
            <div key={index} className="flex justify-between items-center p-4 border-b">
              ...
              <button onClick={() => navigate(`/Cours/update/${index + 1}`)} className="px-3 py-1 rounded bg-[#279D99] text-white">
                Update
              </button>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-lg w-full max-w-3xl p-6 relative border border-[#7ED4D1] shadow-2xl mx-4">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowModal(false)} className="text-[#30B2AC] transition-colors text-2xl font-bold p-1">
                ✕
              </button>
            </div>
              {/* ===== Form ===== */}
              ...
              <div className="flex justify-end gap-4 mt-8">
                <button
                  ...
                  className="border-2 border-[#7ED4D1] px-6 py-2 rounded-lg
                            text-[#1D6F6B] hover:bg-[#DFF6F5] transition"
                >
                  Annuler
                </button>
                <button
