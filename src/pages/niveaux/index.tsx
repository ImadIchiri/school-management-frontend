import { useState } from "react";
import { useNiveaux, type Niveau } from "@/components/niveaux";
import ModalNiveau from "@/components/niveaux/ModalNiveau";

import { FaEdit, FaEye, FaTrash, FaPlus } from "react-icons/fa";
import { FiGrid, FiList } from "react-icons/fi";

const NiveauxDisplay = () => {
  /* ================= STATES ================= */
  const [view, setView] = useState<"grid" | "list">("grid");
  const [showCreate, setShowCreate] = useState(false);
  const [showEdit, setShowEdit] = useState(false);
  const [selectedNiveau, setSelectedNiveau] = useState<Niveau | null>(null);

  const [createErrors, setCreateErrors] = useState<Record<string, string>>({});
  const [editErrors, setEditErrors] = useState<Record<string, string>>({});

  const [currentNiveau, setCurrentNiveau] = useState<Niveau>({
    id: 0,
    anneeLabel: "",
    dateDebut: "",
    dateFin: "",
    filiereId: 0,
  });

  const [newNiveau, setNewNiveau] = useState<Omit<Niveau, "id">>({
    anneeLabel: "",
    dateDebut: "",
    dateFin: "",
    filiereId: 0,
  });

  /* ================= DATA ================= */
  const {
    niveaux,
    loading,
    createNiveau,
    updateNiveau,
    deleteNiveau,
  } = useNiveaux();

  /* ================= CREATE ================= */
  const handleCreate = () => {
    const errors: Record<string, string> = {};

    if (!newNiveau.anneeLabel.trim()) {
      errors.anneeLabel = "L’année est obligatoire";
    }

    if (!newNiveau.filiereId || newNiveau.filiereId === 0) {
      errors.filiereId = "La filière est obligatoire";
    }

    if (Object.keys(errors).length > 0) {
      setCreateErrors(errors);
      return;
    }

    createNiveau(newNiveau);
    setShowCreate(false);
    setCreateErrors({});
    setNewNiveau({
      anneeLabel: "",
      dateDebut: "",
      dateFin: "",
      filiereId: 0,
    });
  };

  /* ================= EDIT ================= */
  const handleEdit = () => {
    const errors: Record<string, string> = {};

    if (!currentNiveau.anneeLabel.trim()) {
      errors.anneeLabel = "L’année est obligatoire";
    }

    if (!currentNiveau.filiereId || currentNiveau.filiereId === 0) {
      errors.filiereId = "La filière est obligatoire";
    }

    if (Object.keys(errors).length > 0) {
      setEditErrors(errors);
      return;
    }

    updateNiveau(currentNiveau.id, currentNiveau);
    setShowEdit(false);
    setEditErrors({});
  };

  /* ================= LOADING ================= */
  if (loading) {
    return (
      <p className="text-center mt-10 text-[#1D6F6B] font-medium">
        Chargement...
      </p>
    );
  }

  /* ================= UI ================= */
  return (
    <div className="p-8 min-h-screen bg-[#DFF6F5]">
      {/* HEADER */}
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-4xl font-bold text-[#1D6F6B]">Niveaux</h1>

        <div className="flex gap-3">
          <button
            onClick={() => setView(view === "grid" ? "list" : "grid")}
            className="w-10 h-10 flex items-center justify-center rounded-lg
                       bg-[#7ED4D1] text-[#1D6F6B]"
          >
            {view === "grid" ? <FiList /> : <FiGrid />}
          </button>

          <button
            onClick={() => setShowCreate(true)}
            className="flex items-center gap-2 px-5 py-3 rounded-xl
                       bg-[#30B2AC] text-white font-semibold"
          >
            <FaPlus /> Créer
          </button>
        </div>
      </div>

      {/* GRID */}
      {view === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {niveaux.map((n) => (
            <div key={n.id} className="bg-white p-6 rounded-xl shadow">
              <h2 className="font-bold text-lg">{n.anneeLabel}</h2>
              <p className="text-sm text-gray-500">Filière ID : {n.filiereId}</p>

              <div className="flex justify-end gap-2 mt-4">
                <button onClick={() => setSelectedNiveau(n)}>
                  <FaEye />
                </button>
                <button
                  onClick={() => {
                    setCurrentNiveau(n);
                    setShowEdit(true);
                  }}
                >
                  <FaEdit />
                </button>
                <button onClick={() => deleteNiveau(n.id)}>
                  <FaTrash className="text-red-500" />
                </button>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* LIST */}
      {view === "list" && (
        <table className="w-full bg-white rounded-xl overflow-hidden shadow">
          <thead className="bg-[#30B2AC] text-white">
            <tr>
              <th className="p-4 text-left">Année</th>
              <th className="p-4 text-left">Filière</th>
              <th className="p-4 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {niveaux.map((n) => (
              <tr key={n.id} className="border-t">
                <td className="p-4">{n.anneeLabel}</td>
                <td className="p-4">{n.filiereId}</td>
                <td className="p-4 text-right">
                  <button onClick={() => setSelectedNiveau(n)}><FaEye /></button>
                  <button
                    onClick={() => {
                      setCurrentNiveau(n);
                      setShowEdit(true);
                    }}
                    className="mx-2"
                  >
                    <FaEdit />
                  </button>
                  <button onClick={() => deleteNiveau(n.id)}>
                    <FaTrash className="text-red-500" />
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      )}

      {/* MODALS */}
      {showCreate && (
        <ModalNiveau
          title="Créer un niveau"
          niveau={newNiveau}
          setNiveau={setNewNiveau}
          onConfirm={handleCreate}
          onClose={() => setShowCreate(false)}
          errors={createErrors}
        />
      )}

      {showEdit && (
        <ModalNiveau
          title="Modifier le niveau"
          niveau={currentNiveau}
          setNiveau={setCurrentNiveau}
          onConfirm={handleEdit}
          onClose={() => setShowEdit(false)}
          errors={editErrors}
        />
      )}
    </div>
  );
};

export default NiveauxDisplay;
